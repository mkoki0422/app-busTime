...
class toukou: Fragment() {
    ...
    override fun onCreateView(
        ...
        val data=toukou_data()



        cal_closest_time(13,data.time as Array<MutableMap<String, Any>>)
        ...
        timer(period = 100){
            ...
            cal_setTime_limitTime(count,data.time as Array<MutableMap<String, Any>>)

            cal_setTimeR_limitTimeR(countR,data.timeR as Array<MutableMap<String, Any>>)
        ...
        return inflater.inflate(R.layout.fragment_toukou, container, false)
...
/*
               if((limitminuteR >= 0 && limitminuteR <= 9) && (limitsecondR >= 0 && limitsecondR <= 9) ){
                ...
                placeR?.text=data.timeR[countR]["place"] as String
                if(countR<13) {
                    nextTimeR?.text = "${data.timeR[countR + 1]["hour"]}時${data.timeR[countR + 1]["minute"]}分"
                }//time[count+1]["place"] as String
                if(countR==13){
                    nextTimeR?.text = " "
                }
                ...
                if(countR>0) {
                    backTimeR?.text = "${data.timeR[countR - 1]["hour"]}時${data.timeR[countR - 1]["minute"]}分"
                ...
                //busTime.text="${sethour}時${setminute}分"
                busTimeR?.text="${data.timeR[countR]["hour"]}時${data.timeR[countR]["minute"]}分"
                ...
                place?.text=data.time[count]["place"] as String
                if(count<13) {
                    nextTime?.text = "${data.time[count + 1]["hour"]}時${data.time[count + 1]["minute"]}分"
                ...
                if(count>0) {
                    backTime?.text = "${data.time[count - 1]["hour"]}時${data.time[count - 1]["minute"]}分"
                }
                if(count==0){
                    backTime?.text=" "
                }
                //busTime.text="${sethour}時${setminute}分"
                busTime?.text="${data.time[count]["hour"]}時${data.time[count]["minute"]}分"
                nowTime?.text = "${hour}時${minute}分${second}秒"
                ...
                minR= (data.timeR[countR]["hour"] as Int)*60*60+(data.timeR[countR]["minute"] as Int)*60
            setsecondR = minR % 60
            setminuteR = minR / 60 % 60
            sethourR = minR / (60 * 60) % 24
            ...
            var limitsecondR = caltimeR % 60
            var limitminuteR = caltimeR / 60 % 60
            var limithourR = caltimeR / (60 * 60) % 24
...
*             val calendar=Calendar.getInstance()//リアルタイムの時間を取得、タイマーを使うことで毎秒更新するため、毎秒動く
            ...
            min= (data.time[count]["hour"] as Int)*60*60+(data.time[count]["minute"] as Int)*60
            setsecond = min % 60
            setminute = min / 60 % 60
            sethour = min / (60 * 60) % 24
            ...
            var limitsecond = caltime % 60
            var limitminute = caltime / 60 % 60
            var limithour = caltime / (60 * 60) % 24
...
/*
*         for (i in 0..13) {
            if ((data.time[i]["hour"] as Int) * 60 * 60 + (data.time[i]["minute"] as Int) * 60 - (hour * 60 * 60 + minute * 60) > 0 && min > (data.time[i]["hour"] as Int) * 60 * 60 + (data.time[i]["minute"] as Int) * 60) {
                min = (data.time[i]["hour"] as Int) * 60 * 60 + (data.time[i]["minute"] as Int) * 60
        ...
        for (R in 0..13) {
            if ((data.timeR[R]["hour"] as Int) * 60 * 60 + (data.timeR[R]["minute"] as Int) * 60 - (hour * 60 * 60 + minute * 60) > 0 && minR > (data.timeR[R]["hour"] as Int) * 60 * 60 + (data.timeR[R]["minute"] as Int) * 60) {

                minR = (data.timeR[R]["hour"] as Int) * 60 * 60 + (data.timeR[R]["minute"] as Int) * 60
                //second 最小の秒を格納してる
                countR = R
